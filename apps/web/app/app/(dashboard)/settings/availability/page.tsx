...
export default async function AvailabilityPage({
  ...
  const slots = await prisma.tenantAvailabilitySlot.findMany({
    where: {
      tenantId: tenant.id,
      date: {
        gte: monthStart,
      ...
      <header className="cf-card cf-hairline p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm cf-muted">
            Imposta la tua disponibilità su date reali del calendario. I clienti
            vedranno questi slot come prenotabili.
          </p>
        </div>
      </header>
